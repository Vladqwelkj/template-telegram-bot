...
def activator(update: Update, context: CallbackContext):
    context.chat_data['selected_date_end_for_compare'] = False
    CalendarKeyboard(context).clear_context_data()
    _send_calendar(update, context, message=messages.DATE_END_SECOND)
...
def select_confirmation(update: Update, context: CallbackContext):
    reply_markup = ReplyKeyboardMarkup([[keyboards.DONE_BUTTON],  [keyboards.BACK_BUTTON]])
    ...
    if context.chat_data['selected_date_end'] and context.chat_data['selected_date_start']:
        if context.chat_data['is_hello_habit_way']:
            ...
            # Здемь нужно отправить сравнение
...
def process_calendar(update: Update, context: CallbackContext):
    ...
    if calendar.is_valid_monthday(update.message.text):
        if not context.chat_data['selected_date_end_for_compare']:
            context.chat_data['selected_date_end_for_compare'] = calendar.get_monthday_date(update.message.text)
...
select_end_date_for_compare_state = State(
